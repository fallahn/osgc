...
//this assumes an xbox controller on windows - the xinput driver for mac
//is also similar. Not tested on linux or with other controllers
//but should detect joystick buttons which are out of range
class JoyMap final
{
public:
    std::string operator[](std::size_t idx) const
    {
        if (idx < m_stringMap.size())
        {
            return m_stringMap[idx];
        }
        return std::to_string(idx);
    }

private:
    const std::array<std::string, 10u> m_stringMap =
    {
        "A", "B", "X", "Y", "LB", "RB", "Back", "Start", "LStick", "RStick"
    };
};
static const JoyMap JoyMapping;
