...
void SpringFlowerSystem::process(float dt)
{
    ...
    for (auto& entity : entities)
    {
        ...
        verts[0].texCoords = { flower.textureRect.left, flower.textureRect.top };
        verts[0].color = flower.colour;

        verts[1].position = verts[0].position;
        verts[1].position.x += flower.textureRect.width;
        verts[1].texCoords = { flower.textureRect.left + flower.textureRect.width, flower.textureRect.top };
        verts[1].color = flower.colour;

        verts[2].position = flower.rootPos;
        verts[2].position.x += flower.textureRect.width / 2.f;
        verts[2].texCoords = { flower.textureRect.left + flower.textureRect.width, flower.textureRect.top  + flower.textureRect.height};
        verts[2].color = flower.colour;

        verts[3].position = verts[2].position;
        verts[3].position.x -= flower.textureRect.width;
        verts[3].texCoords = { flower.textureRect.left, flower.textureRect.top + flower.textureRect.height };
