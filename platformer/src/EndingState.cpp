...
EndingState::EndingState(xy::StateStack& ss, xy::State::Context ctx, SharedData& sd)
    : xy::State (ss, ctx),
...
//public
bool EndingState::handleEvent(const sf::Event& evt)
{
...
void EndingState::handleMessage(const xy::Message& msg)
{
    m_scene.forwardMessage(msg);
}

bool EndingState::update(float dt)
{
    m_scene.update(dt);
    return false;
}
...
void EndingState::build()
{
    ...
    auto fontID = m_resources.load<sf::Font>(FontID::IBMFont);
    ...
    entity = createText("To Be Continued..?");
    entity.getComponent<xy::Transform>().move(0.f, -480.f);

    entity = createText("Credits:");
    entity.getComponent<xy::Transform>().move(0.f, -20.f);

    entity = createText("Gearboy Theme: LittleSadNerd, The Chayed, Kenney, GrafxKid");
    entity.getComponent<xy::Transform>().move(0.f, 80.f);
    entity.getComponent<xy::Text>().setCharacterSize(GameConst::UI::SmallTextSize);
    
    entity = createText("MES Theme: ansimuz, Chromaeleon");
    entity.getComponent<xy::Transform>().move(0.f, 160.f);
    entity.getComponent<xy::Text>().setCharacterSize(GameConst::UI::SmallTextSize);

    entity = createText("Additional artwork, programming: fallahn");
    entity.getComponent<xy::Transform>().move(0.f, 240.f);
    entity.getComponent<xy::Text>().setCharacterSize(GameConst::UI::SmallTextSize);

    entity = createText("Want to contribute?");
    entity.getComponent<xy::Transform>().move(0.f, 320.f);
    entity.getComponent<xy::Text>().setCharacterSize(GameConst::UI::SmallTextSize);
    
    entity = createText("Go to https://github.com/fallahn/osgc to find out how!");
