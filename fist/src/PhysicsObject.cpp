...
void PhysicsObject::applyForceAtLocalPoint(sf::Vector2f force, sf::Vector2f localPos)
{
    XY_ASSERT(m_system && m_body, "Component not initialised!");
    cpBodyApplyForceAtLocalPoint(m_body, Convert::toPhysVec(force), Convert::toPhysVec(localPos));
}

void PhysicsObject::applyImpulseAtLocalPoint(sf::Vector2f impulse, sf::Vector2f localPos)
{
    XY_ASSERT(m_system && m_body, "Component not initialised!");
    cpBodyApplyImpulseAtLocalPoint(m_body, Convert::toPhysVec(impulse), Convert::toPhysVec(localPos));
}

void PhysicsObject::setVelocity(sf::Vector2f velocity)
{
    XY_ASSERT(m_system && m_body, "Component not initialised!");
    cpBodySetVelocity(m_body, Convert::toPhysVec(velocity));
}
